...
async function connect() {
  ...
  const conn = await MongoClient.connect(
    'mongodb+srv://pjosiberto95:josiberto@cluster0.ayuph9r.mongodb.net/?retryWrites=true&w=majority'
  )
...
app.get('/', (req, res) => res.json({ message: 'Funcionou!' }))
...
//josiberto
//POST
app.post('/adm3', async function (req, res, next) {
  try {
    const usuario = req.body
    const db = await connect()
    console.log(usuario)
    res.json(await db.collection('aluno').insertOne(usuario))
  } catch (ex) {
    console.log(ex)
    res.status(400).json({ erro: `${ex}` })
  }  
})
app.get('/adm3/:id', async function (req, res, next) {
  //o "?" depois do :id diz que é um parâmetro opcional
  try {
    const db = await connect()
    if (req.params.id) {
...
router.get("/adm3", async function (req, res, next) {
  //o "?" depois do :id diz que é um parâmetro opcional
  try{
    const db = await connect();
    const collection = await db.collection('aluno');
...
//PUT
app.put('/adm3/:id', async function (req, res, next) {
  try {
    const produto = req.body
